...
// Try to read from Mautic's local.php config
$mauticRoot = null;
$searchPaths = [
    __DIR__ . '/..',
    __DIR__ . '/../..',
...
foreach ($searchPaths as $path) {
    if (file_exists($path . '/app/config/local.php')) {
        $mauticRoot = $path;
        break;
    }
}
...
$config = include $mauticRoot . '/app/config/local.php';
$dbConfig = $config['parameters']['db_driver'] ?? 'mysql';

if ($dbConfig !== 'mysql') {
    print_error("Only MySQL is supported. Found: $dbConfig");
    exit(1);
}

$dbHost = $config['parameters']['db_host'] ?? 'localhost';
$dbPort = $config['parameters']['db_port'] ?? 3306;
$dbName = $config['parameters']['db_name'] ?? 'mautic';
$dbUser = $config['parameters']['db_user'] ?? 'root';
$dbPassword = $config['parameters']['db_password'] ?? '';
